...
(function () {
  ...
  function toast(msg) { return NS.utils.toast(msg); }
  function openCrop(dataUrl, cb, target) { return NS.crop.openCrop(dataUrl, cb, target); }
  ...
  function load() {
    ...
    try {
      ...
      state.settings = setRaw ? JSON.parse(setRaw) : { reviewWeekday: 0, retryDays: 3 };
      if (!state.settings || typeof state.settings.reviewWeekday !== 'number') state.settings = { reviewWeekday: 0, retryDays: 3 };
      if (typeof state.settings.retryDays !== 'number') state.settings.retryDays = 3;
    } catch (e) {
    ...
    try {
      ...
      if (!state.compareCache || typeof state.compareCache !== 'object') state.compareCache = {};
    } catch (e) {
      state.compareCache = {};
    }
    state.summaries = {};
    state.summaryNav = null;
    setTimeout(function () { loadSummariesNow(); }, 600);
  ...
  function loadSummariesNow() {
    ...
    try {
      ...
      if (!state.summaryNav || typeof state.summaryNav !== 'object') state.summaryNav = null;
    } catch (e) {
      state.summaryNav = null;
    }
  }
  ...
  function loadQuestionImages() {
    ...
    function maybeFinish() {
      ...
      if (legacyMigrated) {
        try { storageSet(STORAGE_KEY, JSON.stringify(state.questions, stripQuestionImages)); } catch (e) { /* ignore */ }
      }
    }
    function batch() {
      var n = 0;
      while (k < state.questions.length && n < 4) {
        ...
        if (legacyFormat && q.image && String(q.image).indexOf('data:') === 0) {
          try {
            ...
            legacyMigrated = true;
          } catch (e) { /* image key overflow: keep in memory only */ }
        } else {
          try {
  ...
  function saveSources() {
    try { storageSet('xcapp_sources', JSON.stringify(state.sourceHistory.slice(0, 50))); } catch (e) { /* ignore */ }
  }
  function saveSettings() {
    try { storageSet('xcapp_settings', JSON.stringify(state.settings)); } catch (e) { /* ignore */ }
  }
  function saveCompareCache() {
    try { storageSet('xcapp_shenlun_compare', JSON.stringify(state.compareCache)); } catch (e) { /* ignore */ }
  }
  function saveNewsSaved() {
    try { storageSet('xcapp_news_saved', JSON.stringify(state.news.saved)); } catch (e) { /* ignore */ }
  }
  function saveNewsSummaries() {
    try { storageSet('xcapp_news_summaries', JSON.stringify(state.news.summaries)); } catch (e) { /* ignore */ }
  }
  function saveSummaries() {
    try { storageSet('xcapp_summaries', JSON.stringify(state.summaries)); } catch (e) { /* ignore */ }
  }
  function saveCalcHistory() {
    try { storageSet('xcapp_calc_history', JSON.stringify(state.calc.history)); } catch (e) { /* ignore */ }
  }
  function saveIdioms() {
    try { storageSet('xcapp_idioms', JSON.stringify(state.idiom.saved)); } catch (e) { /* ignore */ }
  }
  function saveAiHistory() {
    try { storageSet('xcapp_ai_history', JSON.stringify(state.ai.history.slice(0, 50))); } catch (e) { /* ignore */ }
  }
    ...
    return jobs.reduce(function (p, job) {
      return p.then(function () {
        ...
        return compressImage(src, maxDim, quality).then(function (out) {
          if (job.kind === 'image') { q.image = out; }
          else if (q.optImgs) { q.optImgs[job.oi] = out; }
          markImgDirty(q.id);
        });
      });
    }, Promise.resolve());
  ...
  function markImgDirty(id) { if (id) state.imgDirty[id] = true; }
  function imgKey(id) { return IMG_KEY_PREFIX + id; }
  function qImagesPayload(q) {
    return JSON.stringify({ image: q.image || null, optImgs: q.optImgs || [], formulaImg: q.formulaImg || null });
  }
  function stripQuestionImages(key, value) {
    if (key === 'image' || key === 'optImgs' || key === 'formulaImg' || key === 'thumb') return undefined;
    return value;
  }
  function persistDirtyImages() {
    ...
    ids.forEach(function (id) {
      var q = findQ(id);
      if (!q) { storageDel(imgKey(id)); return; }
      try { storageSet(imgKey(id), qImagesPayload(q)); } catch (e) { /* image key overflow: keep in memory only */ }
    });
  }
  ...
  function save() {
    ...
    var textOk = false;
    try {
      textOk = storageSet(STORAGE_KEY, JSON.stringify(state.questions, stripQuestionImages));
    } catch (e) { textOk = false; }
    persistDirtyImages();
    if (textOk) return;
    if (saveCompressing) return;
    saveCompressing = true;
    compressQuestionsImages(900, 0.6).then(function () {
      try {
        if (storageSet(STORAGE_KEY, JSON.stringify(state.questions, stripQuestionImages))) {
      ...
      } catch (e2) { /* still full, clear oldest images */ }
      var cleared = 0;
        ...
        if (q && (q.image || (q.optImgs || []).some(Boolean))) {
          ...
          try {
            if (storageSet(STORAGE_KEY, JSON.stringify(state.questions, stripQuestionImages))) {
              ...
              persistDirtyImages();
              render();
              toast('本地存储已满，已清理最旧的 ' + cleared + ' 道题图片（文字保留）');
              return;
            }
          } catch (e3) { /* continue clearing */ }
        }
      }
      saveCompressing = false;
