    ...
    try {
      ...
      var text = await zhipuVision(
        ...
        "识别考试错题截图，返回JSON格式（不要markdown包裹）：" +
          '{"category":"大分类","subCategory":"小分类","stem":"完整题干","options":["A. 内容","B. 内容",...],"answer":"答案字母"}' +
          "【重要】stem必须包含完整的题目文字：" +
          "1. 如果有文段/材料/阅读材料，必须完整保留在stem中，一字不漏，不要省略或概括；" +
          "2. 题目提问句如'下列说法符合这段文字的是''这段文字主要说明''根据这段文字，以下说法正确的是'等必须包含在stem中；" +
          "3. stem = 文段内容 + 提问句，两者缺一不可；" +
          "4. 如果题目没有选项（如填空题、简答题），options留空数组[]，answer留空字符串。" +
          "分类：言语理解(逻辑填空/片段阅读/语句表达)、政治理论(习思想/马克思/时政)、常识判断(经济/科技/人文/地理/法律)、" +
          "判断推理(图形推理/定义判断/类比推理/逻辑判断)、资料分析、数量关系(工程/最值/年龄/行程/几何/排列组合/概率/经济利润等)、申论。" +
          "正确答案：绿色圆圈/对勾标记的选项。直接返回JSON，不要任何解释。",
        2048,
      );
